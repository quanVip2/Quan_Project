...
class PlaylistRepository {
  ...
  Future<int?> playlistNext(BuildContext context, int musicId, int playlistId) async {
    final authState = context.read<AuthBloc>().state;
    String? token;
    if (authState is AuthAuthenticated) {
      token = authState.token;
    }
    final uri = Uri.parse('http://10.0.2.2:8080/app/playlist/next/$musicId?playlistId=$playlistId');
  ...
  Future<int?> playlistRewind(BuildContext context, int musicId, int playlistId) async {
    final authState = context.read<AuthBloc>().state;
    String? token;
    if (authState is AuthAuthenticated) {
      token = authState.token;
    }
    final uri = Uri.parse('http://10.0.2.2:8080/app/playlist/rewind/$musicId?playlistId=$playlistId');
    final resp = await http.get(uri, headers: {
      'Authorization': token != null ? 'Bearer $token' : '',
      'Content-Type': 'application/json',
    });
  ...
  Future<int?> playlistPlay(BuildContext context, int musicId, int playlistId) async {
    final authState = context.read<AuthBloc>().state;
    String? token;
    if (authState is AuthAuthenticated) {
      token = authState.token;
    }
    final uri = Uri.parse('http://10.0.2.2:8080/app/playlist/play/$musicId?playlistId=$playlistId');
    final resp = await http.get(uri, headers: {
      'Authorization': token != null ? 'Bearer $token' : '',
      'Content-Type': 'application/json',
    });
